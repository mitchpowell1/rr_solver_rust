...
#[wasm_bindgen]
pub fn solve(board: js_sys::Array, bots: js_sys::Array, target: usize) -> js_sys::Array {
    let mut game_board: [util::SquareFlags; util::BOARD_SIZE] = [util::SquareFlags::empty(); util::BOARD_SIZE];
    let mut game_bots: [usize; util::BOT_COUNT] = [0; util::BOT_COUNT];
    board
        .iter()
        .enumerate()
    ...
    bots
        .iter()
        .enumerate()
        .for_each(|(i,v)|{game_bots[i] = v.as_f64().unwrap() as usize});
